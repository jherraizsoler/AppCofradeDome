...
class AltaPermisosCofradesActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_altapermisoscofrades)

        etIdCofrade = findViewById(R.id.etIdCofrade_AltaPermisosCofrades)
        etNivelPermiso = findViewById(R.id.etNivelPermiso)
        ...
        btnAlta?.setOnClickListener(View.OnClickListener { v: View? ->
            ...
            var nivelPermiso = 0
            try {
                idCofrade = etIdCofrade?.getText().toString().toInt()
                nivelPermiso = etNivelPermiso?.getText().toString().toInt()
            } catch (e: NumberFormatException) {
                e.printStackTrace()
            }
            val correoElectronico = etCorreoElectronico?.getText().toString()

            // Devuelve los datos a la actividad principal
            val resultado = Intent()
            resultado.putExtra("IDCOFRADE", idCofrade)
            resultado.putExtra("NIVELPERMISO", nivelPermiso)
            resultado.putExtra("CORREOELECTRONICO", correoElectronico)
    ...
    override fun onResume() {
        ...
        if (colorPrincipalPref.length > 0 && rootView != null) {
            ...
            etIdCofrade!!.backgroundTintList = colorStateList
            etNivelPermiso!!.backgroundTintList = colorStateList
            etCorreoElectronico!!.backgroundTintList = colorStateList
        ...
        if (fuenteEncabezadosPref.length > 0) {
            ...
            etIdCofrade!!.typeface = fuenteEncabezados
            etNivelPermiso!!.typeface = fuenteEncabezados
            etCorreoElectronico!!.typeface = fuenteEncabezados
        } else {
            Toast.makeText(
                this,
